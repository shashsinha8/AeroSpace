...
When you use this flag, `tiling`, `floating`, and `sticky` become invalid values for `<target-layout>`.
...
* `tiling|floating|sticky` - Change a window between tiling, floating, and sticky modes.
  Sticky windows float, remain visible when switching workspaces on their current monitor,
  and are mirrored in an always-on-top overlay. Click the overlay to activate the
  real window for native interaction and resizing; the overlay returns when focus
  leaves that window. macOS will ask for Screen Recording permission the first time
  sticky mode is used.
+
...
* Toggle between `sticky` and `tiling` layouts (order of args doesn't matter): +
`aerospace layout sticky tiling`
